...
namespace NkMath{
    ...
    struct Vec4d { 
        ...
        // Déhomogénéisation : passage des coordonnées homogènes au 3D cartésien
        Vec3d ToVec3() const { 
            assert(!nearlyZero(w) && "Impossible de déhomogénéiser: w=0 (direction)"); 
            return {x/w, y/w, z/w}; 
        } 
    ...
    // Projection perspective simple avec paramètres intrinsèques
    ...
    inline Vec3d ProjectToScreen(const Vec4d& v, int w, int h){
        ...
        // Conversion NDC [-1,1] -> coordonnées écran
    ...
    // Vérifie le layout mémoire attendu
