...
namespace NkMath { 
    ...
    struct Mat4d { 
        // Stockage en column-major
        double data[16];     

        // Initialise la matrice à zéro
        Mat4d() { std::fill(data, data+16, 0.0); } 
    
        // Accès (row, col) avec indexation column-major
        ...
        // Multiplication matrice × vecteur homogène
        ...
        // Accès direct au buffer pour upload GPU
        const double* DataPtr() const { return data; } 

        // Conversion vers float pour les API graphiques
        void ToFloat(float out[16]) const { 
            for(int i=0; i<16; i++) out[i] = (float)data[i]; 
        }

        // Rotation autour d'un axe avec la formule de Rodrigues
    ...
    bool Inverse(const Mat4d& m, Mat4d& out) { 
        ...
        for(int col = 0; col < 4; col++) { 
            // Recherche du meilleur pivot dans la colonne courante
            int pivotRow = col; 
            ...
            // Détection d'une matrice non inversible
    ...
    // Comparaison élément par élément avec tolérance
    ...
    Mat4d LookAt(const Vec3d& eye, const Vec3d& target, const Vec3d& worldUp) { 
        ...
        // Axe vertical orthogonal corrigé
        Vec3d u = Cross(r, f); 
    
        // View = rotation + translation dans l'espace caméra
        Mat4d V = Mat4d::Identity(); 
        V(0,0) =  r.x; V(0,1) =  r.y; V(0,2) =  r.z; 
        V(1,0) =  u.x; V(1,1) =  u.y; V(1,2) =  u.z; 
        V(2,0) = -f.x; V(2,1) = -f.y; V(2,2) = -f.z; 

        // Translation exprimée dans la base caméra
        V(0,3) = -Dot(r, eye); 
        V(1,3) = -Dot(u, eye); 
        V(2,3) =  Dot(f, eye);
        return V; 
    ...
    // Construit une Mat4d à partir de [R | t]
    ...
    // Composition TRS à partir d'angles d'Euler
